...
### INV-ANDROID-104 [H] 🐛 Alerte "Connexion perdue" : bandeau permanent, pas toast — découplé de la sonnerie depuis 2026-05-26
...
Donc à introduire un drapeau visuel `heartbeatLostVisual` distinct de `heartbeatLostAlarm` :
...
- **Statut 🐛** : règle adoptée 2026-05-26, à implémenter (aujourd'hui le bandeau et la sonnerie sont armés par le même drapeau).
- **Couverture à refaire** : test09 doit devenir 2 tests, l'un avec réseau présent (bandeau visible, pas de son), l'autre sans réseau (bandeau visible + son). test17 (auth error) inchangé.
...
### INV-ANDROID-302 [C] 🐛 Sonnerie locale "hors connexion" : armée uniquement si perte heartbeat > 2 min ET réseau totalement perdu
...
- **Statut 🐛** : règle business adoptée 2026-05-26 ; code actuel arme la sonnerie sur la seule perte heartbeat (sans check connectivité). À aligner.
- **Couverture à refaire** : test09 (aujourd'hui : heartbeat lost ⇒ bandeau, ne vérifie pas la sonnerie) doit devenir 3 tests :
...
### INV-ANDROID-305 [C] 🐛 Détection "réseau totalement perdu" = ni data ni signal cellulaire
...
- **Statut 🐛** : invariant introduit 2026-05-26 ; aucune détection de connectivité côté client à ce jour. À implémenter.
- **Couverture à créer** :
