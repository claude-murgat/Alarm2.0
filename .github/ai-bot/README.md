...
| 2026-05-12 | #68 | INV-082 (atomicité bulk, re-run) | ✅ **Test concurrence** — 1 writer × 3 readers, mutation injectée (`commit()` après DELETE) → 181 violations détectées. **Premier pick autopilote** (Bloc B cron). | #89 mergée via phase 4 |
| 2026-05-12 | #69 | INV-007 (resolved exclu /active /mine) | ✅ **Verrouillage régression** — 2 tests integration (1 par endpoint), sensibilité prouvée par mutation (ajout `"resolved"` au filtre). | #90 mergée via phase 4 |
| 2026-05-13 | #70 | INV-110 (clamp days [1, 90]) | ✅ **Test paramétré 4 cases** (0, -5, 10, 10000). Exploite asymétrie INV-066 (utcnow vs clock_now) pour vieillir alarme — collision INV-018 documentée. | #91 mergée via phase 4 |
| 2026-05-13 | #71 | INV-074 (refresh nouveau token) | ✅ **2 tests** (token distinct + utilisable). Triplet review primaire+critique+trancheur sur [C]. 2 issues follow-up créées : #96 (path négatif [H]), #97 (clarif stateless JWT). | #92 mergée via phase 4 |
| 2026-05-13 | #73 | INV-077 (admin-only endpoints) | ✅ **1 test paramétré × 5 endpoints**. Stratégie body pydantiquement valide pour éviter faux 422. Mutation manuelle prouvée explicitement. | #93 mergée via phase 4 |
| 2026-05-13 | #72 | INV-078 (logout FCM) | ⚠️ **Abandon→reformulation→succès**. 1er run abandonné (P5 strict sur cas ambigu). Issue reformulée avec wording "verrouille en régression" + PR #94 prompt.md ajoute section canonique. 2e run → 2 tests (cœur + scope filtre device_id). | #95 mergée via phase 4 |

**Bilan 5 pilotes initiaux + 6 autopilote (11 total)** : 1 abandon propre + 1 abandon→reformulation→succès + 9 PRs mergées (mix test régression / fix config / property-based / verrouillage admin). Tous via cycle complet `ai:fix` → agent → `ai:approved` → phase 4 merge auto. Le cron `ai-bot-cron.yml` (4h) pioche désormais automatiquement la plus ancienne `ai:queue` — la session 2026-05-12/13 a mergé 6 PRs sans intervention humaine sur le pipeline (juste les reviews `ai:approved`).
